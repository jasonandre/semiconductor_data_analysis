...
| 구분   | 내용                                            |
| ---- | --------------------------------------------- |
| CMP  | Thickness data 존재, 테스트 이력 2가지 (Dressing step –2 sec / -4 sec )   |
| Etch | Thickness data 존재, 테스트 이력 2가지 (test1 - test2)                    |
| CVD  | Thickness data 없음, 테스트 이력 3가지 (test3 - test5), Chamber A/B/C 존재                |
...
# 3. 박막 두께 기반 CMP·Etch 공정 이상 분석  

Cp=1로 설계한다고 가정했을 때, 타겟 목표를 기준으로 공정이 얼마나 벗어나는지 점검하는 관점   
...
=== [Etch] 전체 LOT 기준 공정 능력 (Spec 기반 σ 기준) ===  
표본 평균 = 4798.16   
표본 표준편차 = 85.73  
Cp   = 1.000  
Cpk  = 0.988  

=== [Etch] 그룹별 Capability 요약 (Spec 기반 σ 기준) ===  
|           Group  |  N   |   Mean   | Cp | Cpk |
| ---------------- | ---- | -------- | --------- | ---------- |
